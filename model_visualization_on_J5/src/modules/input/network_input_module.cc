...
int NetworkInputModule::LoadConfig(const std::string &config_file) {
  int ret = -1;
  std::shared_ptr<JsonConfigWrapper> network_cfg_json = nullptr;

  ret = this->LoadConfigFile(config_file, network_cfg_json);
  if (ret) {
    LOGE << "load config network file failed, ret: " << ret;
    return ret;
  }
  ...
  if (network_cfg_json->HasMember("ip_port")) {
    ...
    LOGE << "ip port parameter is not exist";
    return -1;
  }
  ...
  if (network_cfg_json->HasMember("with_image_name")) {
    ...
    LOGE << "with_image_name parameter is not exist";
    return -1;
  }
...
int NetworkInputModule::Next(std::shared_ptr<PyramidFrame> &output_image) {
  ...
  if (msg_num == 1) {
    for (auto &image : frame->images) {
      ...
      if (size != static_cast<size_t>(height * width * 3 / 2)) {
        LOGE << "nv12 image height: " << height
          << " nv12 image width: " << width
          << " nv12 image size: " << size;
        return -1;
      } else {
    ...
    LOGD << "frame image size: " << frame->images.size();
    for (size_t i = 0; i < frame->images.size(); i += msg_num) {
      ...
      if (last_file == image_name) {
        LOGW << "receive msg aggain, skip it";
        delete frame;
        return false;
      }
      ...
      // image info in msg[1]
      nv12_data =
          reinterpret_cast<char *>(zmq_msg_data(frame->images[i + 1]));
  ...
  if (ret) {
    LOGE << "hb_vio_run_pym failed, ret: " << ret;
    return ret;
  }
  delete frame;
