...
int CodecModule::LoadConfig(const std::string &config_file) {
  ...
  ret = this->LoadConfigFile(config_file, cfg_json);
  if (ret) {
    LOGE << "load codec config file failed, ret: " << ret;
    return ret;
  }

  if (cfg_json->HasMember("codec_chn_num")) {
    chn_num = cfg_json->GetIntValue("codec_chn_num");
  } else {
    LOGE << "codec_chn_num parameter is not exist";
    return -1;
  }

  chn_cfg_list_.resize(MAX_CODEC_INSTANCE);
  for (int i = 0; i < chn_num; i++) {
    std::string chn_num = "chn_" + std::to_string(i);
    auto codec_cfg_json = cfg_json->GetSubConfig(chn_num);
    if (codec_cfg_json == nullptr) {
      LOGE << "chn_" << chn_num << " parameter is not exit!";
      return -1;
    }
    auto codec_cfg = std::make_shared<MediaCodecConfig>();
    if (codec_cfg_json->HasMember("chn_id")) {
      codec_cfg->chn_id = codec_cfg_json->GetIntValue("chn_id");
    } else {
      LOGE << "chn_id parameter is not exist";
      return -1;
    }
    ...
    if (codec_cfg_json->HasMember("frame_buf_count")) {
      ...
      LOGE << "frame_buf_count parameter is not exist";
      return -1;
    }
    if (codec_cfg_json->HasMember("jpeg_quality")) {
      codec_cfg->jpeg_quality = codec_cfg_json->GetIntValue("jpeg_quality");
    } else {
      LOGE << "jpeg_quality parameter is not exist";
      return -1;
    }
    int chn_id = codec_cfg->chn_id;
    chn_cfg_list_[chn_id] = codec_cfg;
    LOGI << "load codec chn_id: " << codec_cfg->chn_id;
...
int CodecModule::ModuleInit(const std::shared_ptr<MediaCodecConfig> &chn_cfg) {
  ...
  chn_ctx_list_[chn_id] = chn_ctx;

  auto context = chn_ctx->context_;
  ret = hb_mm_mc_get_default_context(codec_id, true, context);
  if (ret) {
    LOGE << "hb_mm_mc_get_default_context failed, ret:" << ret;
    return ret;
  }
  LOGD << "get default context: " << context;
...
int CodecModule::ModuleDeInit() {
  int ret = -1;
  int max_chn_num = chn_ctx_list_.size();

  for (int i = 0; i < max_chn_num; i++) {
    std::lock_guard<std::mutex> l(context_mutex_);
    auto chn_ctx = chn_ctx_list_[i];
    if (chn_ctx) {
      auto context = chn_ctx->context_;
      ret = hb_mm_mc_stop(context);
      if (ret) {
        LOGE << "codec chn_id: " << i << " stop failed, ret: " << ret;
        return ret;
      }
      ret = hb_mm_mc_release(context);
      if (ret) {
        LOGE << "codec chn_id: " << i << " release failed, ret: " << ret;
        return ret;
      }
...
int CodecModule::Init(hobot::RunContext *context) {
  int ret = -1;
  // workflow TODO
  // hobot::Workflow *workflow = context->GetWorkflow();

  ret = LoadConfig(config_file_);
  HOBOT_CHECK(ret == 0) << "load config file failed, ret: " << ret;

  chn_ctx_list_.resize(MAX_CODEC_INSTANCE);
  int chn_num = chn_cfg_list_.size();
  for (int i = 0; i < chn_num; i++) {
    auto chn_cfg = chn_cfg_list_[i];
    if (chn_cfg == nullptr) continue;
    auto codec_id = chn_cfg->codec_id;
    // only test J5 jpeg encoder
    if (codec_id == MEDIA_CODEC_ID_JPEG
        || codec_id == MEDIA_CODEC_ID_MJPEG) {
...
FORWARD_DEFINE(CodecModule, 0) {
  hobot::spMessage spRequest = (*input[0])[0];
  ...
  if (GetStream(pym_image, stream_info) == 0
      && stream_info != nullptr) {
    std::shared_ptr<CodecMessage> codec_msg(
        new CodecImageMessage(stream_info),
        [&, this] (CodecImageMessage *p) {
          if (p) {
    ...
    if (codec_msg) {
      ...
      // output = static_cast<hobot::spMessage>(codec_msg);
...
bool CodecModule::FindPymLayer(const int &image_width,
    const int &image_height,
  ...
  bret = FindAvailableImage(image_width, image_height,
      tmp, output_index);
  if (bret == true) {
...
int CodecModule::GetStream(const std::shared_ptr<PyramidFrame> &input,
  ...
  auto chn_cfg = chn_cfg_list_[chn_id];
  if (chn_cfg == nullptr) {
    LOGE << "chn_id: " << chn_id << " chn_cfg is nullptr";
    return -1;
  }
  auto chn_ctx = chn_ctx_list_[chn_id];
  if (chn_ctx == nullptr) {
    LOGE << "chn_id: " << chn_id << " chn_ctx is nullptr";
    return -1;
  }
  auto context = chn_ctx->context_;
  if (context == nullptr) {
  ...
  bret = FindPymLayer(image_width, image_height, input, src_info);
  if (bret == false) {
  ...
  LOGD << "find available pym layer,"
    ...
    << " image_width: " << image_width
    << " image_height: " << image_height
    << " stride: " << src_info.stride
    << " y_paddr: " << src_info.y_paddr
    << " c_paddr: " << src_info.c_paddr
    << " y_vaddr: " << src_info.y_vaddr
  ...
  ret = hb_mm_mc_dequeue_input_buffer(context, &frame_buffer, 2000);
  if (ret) {
...
int CodecModule::FreeStream(const std::shared_ptr<StreamFrame> &input) {
  ...
  HOBOT_CHECK(chn_id == chn_id_val) << "chn_id: " << chn_id
    << " chn_id_val: " << chn_id_val;
  auto context = chn_ctx->context_;
  if (context == nullptr) {
...
void CodecModule::GetDumpNum(const std::string &input,
...
void CodecModule::DumpInputImage(const std::shared_ptr<PyramidFrame> &frame) {
  ...
  LOGD << "codec module input source image,"
    << " chn_id: " << chn_id << " frame_id: " << frame_id
    << " ts: " << ts << " fmt: " << fmt
    << " width: " << width << " height: " << height
    << " stride:"  << stride
    << " y_paddr:" << frame->src_info_.y_paddr
  ...
  if (dump_en == true && (output_dump_count_++ < dump_num || dump_num < 0)) {
    if (fmt == HorizonVisionPixelFormat::kHorizonVisionPixelFormatJPEG ||
      ...
      if (dump_num < 0)
        ...
        file_name = "chn" + std::to_string(chn_id) +
          std::to_string(dump_num) + ".h265";
    } else {
    ...
    //LOGW << "dump fname: " << file_name
      ...
      //<< " frame_size: " << size;
    LOGW << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&: "
      << "dump fname: " << file_name
      << " frame_id: " << frame_id
      << " pts: " << pts
      << " frame_addr: " << reinterpret_cast<void*>(vaddr)
      << " frame_size: " << size;
    std::string root_path = "/deploy/";
    std::string save_path = root_path + file_name;
    std::ofstream ofs(save_path);
    LOGW << "$$$$$$$$$$$$$$$$$$$$ save_path : "  << save_path;
