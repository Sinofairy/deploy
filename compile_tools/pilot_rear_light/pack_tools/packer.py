...
class LmdbDefaultPacker(object):
    ...
    @property
    def anno_transformer(self):
        if self._anno_transformer is unset_flag:
            if isinstance(self.cfg.anno_transformer, list):
                self._anno_transformer = torchvision.transforms.Compose(
                    build_from_registry(self.cfg.anno_transformer)
                )
            elif isinstance(self.cfg.anno_transformer, dict):

                # pprint.pprint(self.cfg.anno_transformer)
                self._anno_transformer = build_from_registry(
                    self.cfg.anno_transformer
                )
        return self._anno_transformer
    ...
    def preprocess_anno_list(self):
        self.logger.info(
            "~~~Preparing anno list for task %s~~~" % self.cfg.task_name
        )

        self.data_packer.preprocess_anno_list()

    def add_anno_transform(self):
        self.data_packer.add_anno_transform(self.anno_transformer)
